...
const clock = await readFile('src/ui/PhysicalChessClock.tsx', 'utf8');
...
for (const cue of ["'move'", "'capture'", "'check'", "'castle'", "'game-start'", "'game-end'", "'clock'"]) {
...
if (!clock.includes("{ haptic: true, sound: false }")) fail('physical-clock haptic must occur on the local press without duplicating audio.');

for (const asset of ['move.mp3', 'capture.wav', 'check.mp3', 'game-start.mp3', 'game-end.wav', 'clock.mp3']) {
...
if (!process.exitCode) console.log('QQURZ audio OK: recorded physical cues only, master mute, tasteful opt-in haptics.');
