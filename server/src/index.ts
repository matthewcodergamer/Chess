...
function remainingFor(room: RoomState, color: Color, at = Date.now()): number {
  ...
  if (room.status !== 'playing' || room.turn !== color || room.turnStartedAt === null) {
...
function makeSnapshot(room: RoomState, connected: Set<Color>, viewerToken: string | null) {
  ...
  const claimedBy = room.coin.claimedByToken
    ? playerForToken(room, room.coin.claimedByToken)?.name ?? null
    : null;
  const winner = room.coin.winnerToken
    ? playerForToken(room, room.coin.winnerToken)?.name ?? null
    : null;
  const yourFace = viewerToken && room.coin.claimedByToken && room.coin.claimedFace
    ? viewerToken === room.coin.claimedByToken
      ? room.coin.claimedFace
      : oppositeFace(room.coin.claimedFace)
    : null;
  ...
  return {
    ...
    auction: {
      ...
      leaderName: room.auction.leaderToken
        ? playerForToken(room, room.auction.leaderToken)?.name ?? null
        : null,
...
function parseRoomPath(pathname: string): { code: string; action: 'join' | 'ws' } | null {
  ...
  return {
    ...
    action: match[2].toLowerCase() as 'join' | 'ws',
  };
}
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (route.action === 'ws' && request.method === 'GET') {
...
export class ChessRoom extends DurableObject<Env> {
  ...
  async fetch(request: Request): Promise<Response> {
    ...
    if (url.hostname === 'room.internal' && url.pathname === '/create' && request.method === 'POST') {
      ...
      this.room = {
        ...
        lastActivityAt: now,
        players: {
          white: { name: normalizeName(body.name), token },
          black: null,
        },
        coin: emptyCoin(),
  ...
  async webSocketMessage(ws: WebSocket, message: string | ArrayBuffer): Promise<void> {
    ...
    if (payload.type === 'call_coin') {
      await this.callCoin(ws, attachment.token, payload.face);
      return;
    }

    if (payload.type === 'claim_position_bid') {
      await this.claimPositionBid(ws, attachment.token, payload.sessionId);
      return;
    }

    if (payload.type === 'move') {
      await this.handleMove(ws, color, payload.uci);
      return;
    }
    ...
    if (payload.type === 'resign') {
      if (this.room.status !== 'playing') {
        return this.sendError(ws, 'There is no active game to resign.');
      }
  ...
  private sendSnapshot(ws: WebSocket, token: string): void {
    try {
      ws.send(JSON.stringify({ type: 'snapshot', room: this.snapshotForToken(token) }));
    } catch {
  ...
  private async callCoin(ws: WebSocket, token: string, face: CoinFace): Promise<void> {
    if (!this.room || this.room.status !== 'coin' || !this.room.players.black) {
      return this.sendError(ws, 'The coin toss is not available right now.');
    }
    if (face !== 'heads' && face !== 'tails') {
      return this.sendError(ws, 'Choose Heads or Tails.');
    }
    if (this.room.coin.result || this.room.coin.claimedByToken) {
      return this.sendError(ws, 'Another player already called the coin.');
    }
    ...
    const otherToken = this.room.players.white.token === token
      ? this.room.players.black.token
      : this.room.players.white.token;
    ...
    this.room.coin.winnerToken = winnerToken;
  ...
  private async verifyBidSession(sessionId: string): Promise<{ cents: number }> {
    ...
    if (mode === 'live' && this.env.LIVE_POSITION_BIDS !== 'enabled') {
      throw new Error('Live paid position bidding is disabled. Use test mode until the competitive rules are approved.');
    }

    const pattern = mode === 'live'
      ? /^cs_live_[A-Za-z0-9_]+$/
      : /^cs_test_[A-Za-z0-9_]+$/;
    ...
    if (
      ...
      || payload.metadata?.kind !== 'position_bid'
      || payload.metadata?.room_code !== this.room.code
      || !BID_VALUES.has(cents)
    ) {
      throw new Error('This payment does not match a valid QQURZ position bid for this room.');
    }
  ...
  private async claimPositionBid(ws: WebSocket, token: string, rawSessionId: unknown): Promise<void> {
    if (!this.room || (this.room.status !== 'coin' && this.room.status !== 'strategy')) {
      return this.sendError(ws, 'Position bidding closes when play begins.');
    }

    const sessionId = String(rawSessionId ?? '').trim();
    if (!sessionId) return this.sendError(ws, 'Missing Checkout Session.');
    if (this.room.auction.usedSessions.includes(sessionId)) {
      return this.sendError(ws, 'That bid payment has already been claimed.');
    }

    try {
      const { cents } = await this.verifyBidSession(sessionId);
      const now = Date.now();
      const previous = this.room.auction.bids[token]?.cents ?? 0;
      if (cents <= previous) {
        return this.sendError(ws, 'Your new bid must be higher than your previous verified bid.');
      }
      ...
      if (cents > this.room.auction.leadingBidCents) {
        this.room.auction.leadingBidCents = cents;
        this.room.auction.leaderToken = token;
        this.rerollPosition();
        this.room.auction.rerollCount += 1;
      }
      ...
      this.sendError(ws, error instanceof Error ? error.message : 'Could not verify the paid bid.');
    }
  }
  ...
  private settleActiveClock(now: number): void {
    ...
    if (this.room.turn === 'white') {
      this.room.whiteClockMs = Math.max(0, this.room.whiteClockMs - elapsed);
    } else {
      this.room.blackClockMs = Math.max(0, this.room.blackClockMs - elapsed);
    }
    this.room.turnStartedAt = now;
  }
  ...
  private async handleMove(ws: WebSocket, color: Color, rawUci: unknown): Promise<void> {
    if (!this.room) return;
    if (this.room.status !== 'playing' || this.room.result) {
      return this.sendError(ws, 'The game is not accepting moves.');
    }
    ...
    if (!/^[a-h][1-8][a-h][1-8][qrbn]?$/.test(uci)) {
      return this.sendError(ws, 'Invalid move format.');
    }
