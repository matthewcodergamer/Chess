...
type RoomState = {
  code: string; status: RoomStatus; positionId: number; fen: string; turn: Color;
  whiteClockMs: number; blackClockMs: number; turnStartedAt: number | null; strategyEndsAt: number | null;
  moves: string[]; result: string | null; check: boolean; checkmate: boolean; createdAt: number; lastActivityAt: number;
  players: { white: PlayerSeat; black: PlayerSeat | null };
  coin: CoinState; auction: AuctionState;
};
...
const emptyCoin = (): CoinState => ({ claimedFace: null, claimedByToken: null, result: null, winnerToken: null, flippedAt: null, endsAt: null });
const emptyAuction = (): AuctionState => ({ bids: {}, leaderToken: null, leadingBidCents: 0, rerollCount: 0, usedSessions: [] });
...
function randomCoin(): CoinFace { const b = new Uint8Array(1); crypto.getRandomValues(b); return b[0] % 2 === 0 ? 'heads' : 'tails'; }
...
function allowedOrigins(env: Env): Set<string> { return new Set((env.ALLOWED_ORIGINS ?? '').split(',').map(v => v.trim()).filter(Boolean)); }
...
function makeSnapshot(room: RoomState, connected: Set<Color>, viewerToken: string | null) {
  const now = Date.now(); const viewerColor = viewerToken ? colorForToken(room, viewerToken) : null;
  ...
  const yourFace = viewerToken && room.coin.claimedByToken && room.coin.claimedFace
    ? viewerToken === room.coin.claimedByToken ? room.coin.claimedFace : oppositeFace(room.coin.claimedFace) : null;
  return {
    ...
    turnStartedAt: room.turnStartedAt, strategyEndsAt: room.strategyEndsAt, serverNow: now, moves: room.moves,
    result: room.result, check: room.check, checkmate: room.checkmate, yourColor: viewerColor,
    players: {
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (request.method === 'POST' && url.pathname === '/rooms') {
      const body = await request.json().catch(() => ({})) as Record<string, unknown>; const name = normalizeName(body.name);
      for (let attempt = 0; attempt < 12; attempt += 1) {
        ...
        if (internal.status === 409) continue; return cors(request, internal, env);
      }
      return cors(request, json({ error: 'Could not allocate a room code. Try again.' }, 503), env);
    }
    const route = parseRoomPath(url.pathname); if (!route) return cors(request, json({ error: 'Not found.' }, 404), env);
    const stub = env.ROOMS.get(env.ROOMS.idFromName(route.code));
    if (route.action === 'join' && request.method === 'POST') {
      const body = await request.json().catch(() => ({})) as Record<string, unknown>;
      return cors(request, await stub.fetch(new Request('https://room.internal/join', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ name: normalizeName(body.name) }) })), env);
    }
...
export class ChessRoom extends DurableObject<Env> {
  ...
  async fetch(request: Request): Promise<Response> {
    ...
    if (url.hostname === 'room.internal' && url.pathname === '/create' && request.method === 'POST') {
      ...
      const body = await request.json() as { code?: string; name?: string }; const code = String(body.code ?? '').toUpperCase();
      if (!/^[A-Z0-9]{6}$/.test(code)) return json({ error: 'Invalid room code.' }, 400);
      const now = Date.now(); const token = seatToken(); const positionId = randomChess960Id();
      this.room = {
        code, status: 'waiting', positionId, fen: chess960Fen(positionId), turn: 'white', whiteClockMs: GAME_CLOCK_MS, blackClockMs: GAME_CLOCK_MS,
        turnStartedAt: null, strategyEndsAt: null, moves: [], result: null, check: false, checkmate: false, createdAt: now, lastActivityAt: now,
        players: { white: { name: normalizeName(body.name), token }, black: null }, coin: emptyCoin(), auction: emptyAuction(),
      };
      await this.persist(); return json({ code, token, color: 'white' });
    }
    if (url.hostname === 'room.internal' && url.pathname === '/join' && request.method === 'POST') {
      ...
      this.room.players.black = { name: normalizeName(body.name), token }; this.prepareCoin(Date.now());
      await this.persist(); await this.scheduleForState(); this.broadcast(); return json({ code: this.room.code, token, color: 'black' });
    }
    if (request.headers.get('upgrade')?.toLowerCase() === 'websocket') {
      if (!this.room) return json({ error: 'That room does not exist.' }, 404);
      const token = url.searchParams.get('token') ?? ''; if (!colorForToken(this.room, token)) return json({ error: 'Invalid room seat token.' }, 401);
      const pair = new WebSocketPair(); const client = pair[0]; const server = pair[1]; this.ctx.acceptWebSocket(server);
      server.serializeAttachment({ token } satisfies SocketAttachment); this.sendSnapshot(server, token); queueMicrotask(() => this.broadcast());
  ...
  async webSocketMessage(ws: WebSocket, message: string | ArrayBuffer): Promise<void> {
    ...
    const attachment = ws.deserializeAttachment() as SocketAttachment | null; const color = attachment ? colorForToken(this.room, attachment.token) : null;
    ...
    if (payload.type === 'call_coin') return this.callCoin(ws, attachment.token, payload.face);
    if (payload.type === 'claim_position_bid') return this.claimPositionBid(ws, attachment.token, payload.sessionId);
    if (payload.type === 'move') return this.handleMove(ws, color, payload.uci);
    ...
    if (payload.type === 'resign') {
      ...
      this.room.turnStartedAt = null; this.room.lastActivityAt = Date.now(); await this.persist(); await this.ctx.storage.deleteAlarm(); this.broadcast(); return;
    }
  ...
  async alarm(): Promise<void> {
    ...
    if (this.room.status === 'playing') {
      this.settleActiveClock(now); const remaining = this.room.turn === 'white' ? this.room.whiteClockMs : this.room.blackClockMs;
      if (remaining <= 0) { this.room.status = 'ended'; this.room.result = `${opposite(this.room.turn) === 'white' ? 'White' : 'Black'} wins on time`; this.room.turnStartedAt = null; await this.persist(); this.broadcast(); return; }
  ...
  private connectedColors(): Set<Color> {
    ...
    for (const ws of this.ctx.getWebSockets()) { const a = ws.deserializeAttachment() as SocketAttachment | null; const c = a ? colorForToken(this.room, a.token) : null; if (c) colors.add(c); }
    return colors;
  }
  ...
  private sendSnapshot(ws: WebSocket, token: string): void { try { ws.send(JSON.stringify({ type: 'snapshot', room: this.snapshotForToken(token) })); } catch { /* closing */ } }
  private broadcast(): void { if (!this.room) return; for (const ws of this.ctx.getWebSockets()) { const a = ws.deserializeAttachment() as SocketAttachment | null; if (a?.token) this.sendSnapshot(ws, a.token); } }
  private sendError(ws: WebSocket, message: string): void { try { ws.send(JSON.stringify({ type: 'error', message })); } catch { /* closing */ } }

  private prepareCoin(now: number): void {
    if (!this.room) return;
    this.room.status = 'coin'; this.room.turnStartedAt = null; this.room.strategyEndsAt = null; this.room.whiteClockMs = GAME_CLOCK_MS; this.room.blackClockMs = GAME_CLOCK_MS;
    this.room.moves = []; this.room.result = null; this.room.check = false; this.room.checkmate = false; this.room.coin = emptyCoin(); this.room.auction = emptyAuction(); this.room.lastActivityAt = now;
  }
  ...
  private async callCoin(ws: WebSocket, token: string, face: CoinFace): Promise<void> {
    ...
    const result = randomCoin(); const otherToken = this.room.players.white.token === token ? this.room.players.black.token : this.room.players.white.token;
    const winnerToken = result === face ? token : otherToken;
    if (this.room.players.white.token !== winnerToken && this.room.players.black) { const oldWhite = this.room.players.white; this.room.players.white = this.room.players.black; this.room.players.black = oldWhite; }
  ...
  private paymentMode(): 'off' | 'test' | 'live' {
    ...
    if (this.env.PAYMENTS_MODE === 'live' && key.startsWith('sk_live_')) return 'live'; return 'off';
  }
  private async verifyBidSession(sessionId: string): Promise<{ cents: number }> {
    if (!this.room) throw new Error('Room unavailable.'); const mode = this.paymentMode();
    ...
    const pattern = mode === 'live' ? /^cs_live_[A-Za-z0-9_]+$/ : /^cs_test_[A-Za-z0-9_]+$/; if (!pattern.test(sessionId)) throw new Error('Invalid Checkout Session.');
  ...
  private async claimPositionBid(ws: WebSocket, token: string, rawSessionId: unknown): Promise<void> {
    ...
    const sessionId = String(rawSessionId ?? '').trim(); if (!sessionId) return this.sendError(ws, 'Missing Checkout Session.');
    if (this.room.auction.usedSessions.includes(sessionId)) return this.sendError(ws, 'That bid payment has already been claimed.');
    try {
      const { cents } = await this.verifyBidSession(sessionId); const now = Date.now(); const previous = this.room.auction.bids[token]?.cents ?? 0;
      if (cents <= previous) return this.sendError(ws, 'Your new bid must be higher than your previous verified bid.');
      this.room.auction.usedSessions = [...this.room.auction.usedSessions, sessionId].slice(-24); this.room.auction.bids[token] = { cents, paidAt: now, sessionId };
  ...
  private rerollPosition(): void {
    if (!this.room) return; const positionId = randomChess960Id(); this.room.positionId = positionId; this.room.fen = chess960Fen(positionId); this.room.turn = 'white'; this.room.moves = []; this.room.result = null; this.room.check = false; this.room.checkmate = false;
  }
  private startPlaying(now: number): void { if (!this.room) return; this.room.status = 'playing'; this.room.strategyEndsAt = null; this.room.turnStartedAt = now; this.room.lastActivityAt = now; }
  private settleActiveClock(now: number): void {
    if (!this.room || this.room.status !== 'playing' || this.room.turnStartedAt === null) return; const elapsed = Math.max(0, now - this.room.turnStartedAt);
    if (this.room.turn === 'white') this.room.whiteClockMs = Math.max(0, this.room.whiteClockMs - elapsed); else this.room.blackClockMs = Math.max(0, this.room.blackClockMs - elapsed); this.room.turnStartedAt = now;
  }
  private async handleMove(ws: WebSocket, color: Color, rawUci: unknown): Promise<void> {
    if (!this.room) return; if (this.room.status !== 'playing' || this.room.result) return this.sendError(ws, 'The game is not accepting moves.'); if (this.room.turn !== color) return this.sendError(ws, 'It is not your turn.');
    const now = Date.now(); this.settleActiveClock(now); const currentClock = color === 'white' ? this.room.whiteClockMs : this.room.blackClockMs;
    if (currentClock <= 0) { this.room.status = 'ended'; this.room.result = `${opposite(color) === 'white' ? 'White' : 'Black'} wins on time`; this.room.turnStartedAt = null; await this.persist(); await this.ctx.storage.deleteAlarm(); this.broadcast(); return; }
    const uci = String(rawUci ?? '').trim().toLowerCase(); if (!/^[a-h][1-8][a-h][1-8][qrbn]?$/.test(uci)) return this.sendError(ws, 'Invalid move format.');
    let position: Chess; try { position = Chess.fromSetup(parseFen(this.room.fen).unwrap()).unwrap(); } catch { return this.sendError(ws, 'The server could not read the room position.'); }
    const move = parseUci(uci); if (!move || !position.isLegal(move)) { this.room.turnStartedAt = now; await this.persist(); await this.scheduleForState(); return this.sendError(ws, 'That move is not legal.'); }
    const san = makeSan(position, move); position.play(move); this.room.fen = makeFen(position.toSetup()); this.room.turn = position.turn; this.room.moves = [...this.room.moves, san].slice(-400); this.room.check = position.isCheck(); this.room.checkmate = position.isCheckmate(); this.room.lastActivityAt = now;
    const ending = resultText(position); if (ending) { this.room.status = 'ended'; this.room.result = ending; this.room.turnStartedAt = null; await this.ctx.storage.deleteAlarm(); } else this.room.turnStartedAt = now;
    await this.persist(); await this.scheduleForState(); this.broadcast();
  }
  ...
  private async scheduleForState(): Promise<void> {
    if (!this.room) return;
    if (this.room.status === 'coin' && this.room.coin.endsAt) return this.ctx.storage.setAlarm(this.room.coin.endsAt);
    if (this.room.status === 'strategy' && this.room.strategyEndsAt) return this.ctx.storage.setAlarm(this.room.strategyEndsAt);
    if (this.room.status === 'playing' && this.room.turnStartedAt !== null) { const remaining = this.room.turn === 'white' ? this.room.whiteClockMs : this.room.blackClockMs; return this.ctx.storage.setAlarm(Date.now() + Math.max(1, remaining)); }
