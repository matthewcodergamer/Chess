...
export async function applyDataCommands(env: DataModelEnv, commands: DataCommand[]): Promise<void> {
  ...
  if (commands.length > MAX_COMMANDS) throw new Error(`Data command batch exceeds ${MAX_COMMANDS}.`);
  const response = await dataRegistryStub(env).fetch(new Request('https://data.internal/internal/model/commands', {
    method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ commands } satisfies DataCommandBatch),
  }));
...
export class DataRegistry extends DurableObject<DataModelEnv> {
      ...
      sql.exec(
        `INSERT INTO games(id,room_code,status,result_kind,result_text,winner_user_id,position_id,initial_fen,final_fen,base_ms,increment_ms,rated,rating_pool,tournament_id,tournament_round_id,pairing_id,created_at,started_at,ended_at,metadata_json)
         ...
         ON CONFLICT(id) DO UPDATE SET status=excluded.status,result_kind=excluded.result_kind,result_text=excluded.result_text,winner_user_id=excluded.winner_user_id,
           final_fen=excluded.final_fen,rated=excluded.rated,rating_pool=excluded.rating_pool,tournament_id=excluded.tournament_id,
           tournament_round_id=excluded.tournament_round_id,pairing_id=excluded.pairing_id,started_at=excluded.started_at,ended_at=excluded.ended_at,metadata_json=excluded.metadata_json`,
        ...
        sql.exec(
          `INSERT INTO game_participants(id,game_id,user_id,seat_no,color,display_name_snapshot,outcome,rating_pool,rating_before,rating_after,rating_deviation_before,rating_deviation_after,joined_at,left_at)
           ...
           ON CONFLICT(game_id,seat_no) DO UPDATE SET user_id=excluded.user_id,color=excluded.color,display_name_snapshot=excluded.display_name_snapshot,
             outcome=excluded.outcome,rating_pool=excluded.rating_pool,rating_before=excluded.rating_before,rating_after=excluded.rating_after,
             rating_deviation_before=excluded.rating_deviation_before,rating_deviation_after=excluded.rating_deviation_after,left_at=excluded.left_at`,
      ...
      for (const move of game.moves ?? []) {
        sql.exec(
          `INSERT INTO game_moves(game_id,ply,mover_user_id,mover_color,uci,san,fen_after,client_sent_at,server_received_at,committed_at,think_ms,clock_after_ms,created_at)
           VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)
           ON CONFLICT(game_id,ply) DO UPDATE SET mover_user_id=excluded.mover_user_id,uci=excluded.uci,san=excluded.san,fen_after=excluded.fen_after,
             client_sent_at=excluded.client_sent_at,server_received_at=excluded.server_received_at,committed_at=excluded.committed_at,think_ms=excluded.think_ms,clock_after_ms=excluded.clock_after_ms`,
  ...
  private applyLedger(command: Extract<DataCommand, { type: 'ledger_transaction' }>): void {
    ...
    this.ctx.storage.transactionSync(() => {
      const cursor = this.ctx.storage.sql.exec(
        `INSERT INTO ledger_transactions(id,transaction_type,purpose,reference,idempotency_key,provider,provider_reference,status,sequence,previous_hash,hash,metadata_json,created_at,reversed_by_transaction_id)
         VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(idempotency_key) DO NOTHING`,
        tx.id, tx.transactionType, tx.purpose, tx.reference ?? '', tx.idempotencyKey, tx.provider ?? null, tx.providerReference ?? null, tx.status ?? 'posted',
        finite(tx.sequence), tx.previousHash ?? null, tx.hash ?? null, jsonText(tx.metadata), tx.createdAt, tx.reversedByTransactionId ?? null,
      );
  ...
  private applyCommand(command: DataCommand): void {
    ...
    switch (command.type) {
      ...
      case 'revoke_session':
        sql.exec(`UPDATE sessions SET revoked_at=?,revoke_reason=? WHERE id=?`, command.revokedAt, command.reason ?? null, command.sessionId); return;
      ...
      case 'payment_customer': {
        ...
        sql.exec(`INSERT INTO payment_customers(id,user_id,provider,provider_customer_id,created_at,updated_at) VALUES(?,?,?,?,?,?)
          ON CONFLICT(user_id,provider) DO UPDATE SET provider_customer_id=excluded.provider_customer_id,updated_at=excluded.updated_at`,c.id,c.userId,c.provider,c.providerCustomerId,c.createdAt,c.updatedAt); return;
      }
      case 'payment_intent': {
        const i = command.intent;
        sql.exec(`INSERT INTO payment_intents(id,user_id,customer_id,provider,provider_intent_id,purpose,amount_cents,currency,status,idempotency_key,game_id,tournament_id,metadata_json,created_at,updated_at,confirmed_at)
          VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(idempotency_key) DO UPDATE SET provider_intent_id=excluded.provider_intent_id,status=excluded.status,metadata_json=excluded.metadata_json,updated_at=excluded.updated_at,confirmed_at=excluded.confirmed_at`,
          i.id,i.userId ?? null,i.customerId ?? null,i.provider,i.providerIntentId ?? null,i.purpose,i.amountCents,i.currency ?? 'USD',i.status,i.idempotencyKey,i.gameId ?? null,i.tournamentId ?? null,jsonText(i.metadata),i.createdAt,i.updatedAt,finite(i.confirmedAt)); return;
      }
      case 'wallet_account': {
        const w = command.wallet;
        sql.exec(`INSERT INTO wallet_accounts(id,user_id,account_type,currency,status,created_at,updated_at) VALUES(?,?,?,?,?,?,?)
          ON CONFLICT(id) DO UPDATE SET status=excluded.status,updated_at=excluded.updated_at`,w.id,w.userId ?? null,w.accountType,w.currency ?? 'USD',w.status ?? 'open',w.createdAt,w.updatedAt); return;
      }
      case 'ledger_transaction': return this.applyLedger(command);
      case 'payout': {
        const p=command.payout;
        sql.exec(`INSERT INTO payouts(id,user_id,wallet_account_id,provider,provider_payout_id,amount_cents,currency,status,idempotency_key,requested_at,processed_at,failure_reason,metadata_json) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)
          ON CONFLICT(idempotency_key) DO UPDATE SET provider_payout_id=excluded.provider_payout_id,status=excluded.status,processed_at=excluded.processed_at,failure_reason=excluded.failure_reason,metadata_json=excluded.metadata_json`,
          p.id,p.userId,p.walletAccountId,p.provider,p.providerPayoutId ?? null,p.amountCents,p.currency ?? 'USD',p.status,p.idempotencyKey,p.requestedAt,finite(p.processedAt),p.failureReason ?? null,jsonText(p.metadata)); return;
      }
      case 'refund': {
        const r=command.refund;
        sql.exec(`INSERT INTO refunds(id,user_id,payment_intent_id,ledger_transaction_id,provider,provider_refund_id,amount_cents,currency,status,reason,idempotency_key,created_at,processed_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)
          ON CONFLICT(idempotency_key) DO UPDATE SET provider_refund_id=excluded.provider_refund_id,status=excluded.status,processed_at=excluded.processed_at`,
          r.id,r.userId ?? null,r.paymentIntentId ?? null,r.ledgerTransactionId ?? null,r.provider,r.providerRefundId ?? null,r.amountCents,r.currency ?? 'USD',r.status,r.reason ?? '',r.idempotencyKey,r.createdAt,finite(r.processedAt)); return;
      }
      case 'subscription': {
        const s=command.subscription;
        sql.exec(`INSERT INTO subscriptions(id,user_id,provider,provider_subscription_id,plan_key,status,current_period_start,current_period_end,cancel_at_period_end,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?)
          ON CONFLICT(id) DO UPDATE SET provider_subscription_id=excluded.provider_subscription_id,plan_key=excluded.plan_key,status=excluded.status,current_period_start=excluded.current_period_start,current_period_end=excluded.current_period_end,cancel_at_period_end=excluded.cancel_at_period_end,updated_at=excluded.updated_at`,
          s.id,s.userId,s.provider,s.providerSubscriptionId ?? null,s.planKey,s.status,finite(s.currentPeriodStart),finite(s.currentPeriodEnd),bool(s.cancelAtPeriodEnd),s.createdAt,s.updatedAt); return;
      }
      case 'entitlement': {
        const e=command.entitlement;
        sql.exec(`INSERT INTO premium_entitlements(id,user_id,entitlement_key,source_type,source_id,status,starts_at,ends_at,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?)
          ON CONFLICT(id) DO UPDATE SET status=excluded.status,starts_at=excluded.starts_at,ends_at=excluded.ends_at,updated_at=excluded.updated_at`,e.id,e.userId,e.entitlementKey,e.sourceType,e.sourceId ?? null,e.status,e.startsAt,finite(e.endsAt),e.createdAt,e.updatedAt); return;
      }
      case 'moderation_report': {
        const r=command.report;
        sql.exec(`INSERT INTO moderation_reports(id,reporter_user_id,target_user_id,game_id,tournament_id,category,narrative,status,priority,created_at,updated_at,reviewed_at,reviewed_by_user_id,metadata_json) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)
          ON CONFLICT(id) DO UPDATE SET status=excluded.status,priority=excluded.priority,updated_at=excluded.updated_at,reviewed_at=excluded.reviewed_at,reviewed_by_user_id=excluded.reviewed_by_user_id,metadata_json=excluded.metadata_json`,
      ...
      case 'moderation_record': {
        ...
        sql.exec(`INSERT INTO moderation_records(id,user_id,report_id,action,reason,starts_at,ends_at,created_at,created_by_user_id,metadata_json) VALUES(?,?,?,?,?,?,?,?,?,?)
          ON CONFLICT(id) DO UPDATE SET ends_at=excluded.ends_at,metadata_json=excluded.metadata_json`,r.id,r.userId ?? null,r.reportId ?? null,r.action,r.reason,r.startsAt,finite(r.endsAt),r.createdAt,r.createdByUserId ?? null,jsonText(r.metadata)); return;
      }
      case 'device': {
        const d=command.device;
        sql.exec(`INSERT INTO device_records(id,user_id,fingerprint_hash,device_name,platform,browser,first_seen_at,last_seen_at,trusted_at,revoked_at,metadata_json) VALUES(?,?,?,?,?,?,?,?,?,?,?)
          ON CONFLICT(id) DO UPDATE SET user_id=excluded.user_id,device_name=excluded.device_name,platform=excluded.platform,browser=excluded.browser,last_seen_at=excluded.last_seen_at,trusted_at=excluded.trusted_at,revoked_at=excluded.revoked_at,metadata_json=excluded.metadata_json`,
      ...
      case 'security_event': {
        ...
        sql.exec(`INSERT OR IGNORE INTO security_events(id,user_id,session_id,device_record_id,event_type,success,risk_score,ip_prefix,user_agent,metadata_json,created_at) VALUES(?,?,?,?,?,?,?,?,?,?,?)`,
          e.id,e.userId ?? null,e.sessionId ?? null,e.deviceRecordId ?? null,e.eventType,bool(e.success,true),Math.max(0,Math.min(100,integer(e.riskScore))),e.ipPrefix ?? '',e.userAgent ?? '',jsonText(e.metadata),e.createdAt); return;
      }
      case 'notification': {
        const n=command.notification;
        sql.exec(`INSERT OR IGNORE INTO notifications(id,user_id,notification_type,title,body,data_json,dedupe_key,created_at,read_at,delivered_at,push_attempted_at,expires_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)`,
          n.id,n.userId,n.notificationType,n.title,n.body,jsonText(n.data),n.dedupeKey ?? null,n.createdAt,finite(n.readAt),finite(n.deliveredAt),finite(n.pushAttemptedAt),finite(n.expiresAt));
        sql.exec(`UPDATE notifications SET read_at=?,delivered_at=?,push_attempted_at=?,expires_at=? WHERE id=?`,finite(n.readAt),finite(n.deliveredAt),finite(n.pushAttemptedAt),finite(n.expiresAt),n.id); return;
      }
      case 'audit': {
        const e=command.event;
        sql.exec(`INSERT OR IGNORE INTO audit_logs(event_id,actor_user_id,actor_type,action,entity_type,entity_id,request_id,ip_prefix,before_json,after_json,metadata_json,created_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)`,
          e.eventId,e.actorUserId ?? null,e.actorType,e.action,e.entityType,e.entityId,e.requestId ?? null,e.ipPrefix ?? '',e.before === undefined ? null : jsonText(e.before),e.after === undefined ? null : jsonText(e.after),jsonText(e.metadata),e.createdAt); return;
      }
    }
  }
  ...
  private applyBatch(batch: DataCommandBatch): Response {
    ...
    if (batch.commands.length > MAX_COMMANDS) return json({ error: `Too many commands; max ${MAX_COMMANDS}.` }, 413);
    try {
      this.ctx.storage.transactionSync(() => {
        for (const command of batch.commands) this.applyCommand(command);
      });
      return json({ ok: true, applied: batch.commands.length, schemaVersion: DATA_SCHEMA_VERSION });
    } catch (error) {
  ...
  private health(): Response {
    ...
    const missing = REQUIRED_DATA_TABLES.filter(name => !names.has(name));
    return json({ ok: missing.length === 0, schemaVersion: DATA_SCHEMA_VERSION, tables: REQUIRED_DATA_TABLES.length, missing });
  }
