...
function normalizeBidCents(value: unknown): number {
  const cents = Math.round(Number(value));
  return Number.isFinite(cents) && cents >= COLOR_BID_MIN_CENTS && cents <= COLOR_BID_MAX_CENTS ? cents : 0;
}
function normalizeBidColor(value: unknown): BidColor | '' { return value === 'white' || value === 'black' ? value : ''; }
function itemFor(kind: CheckoutKind, itemId: string, env: TournamentEnv, bidCents = 0, desiredColor: BidColor | '' = '') {
  ...
  if (kind === 'color_bid' && itemId === 'color-bid' && bidCents && desiredColor) return { id: itemId, name: `QQURZ ${desiredColor === 'white' ? 'White' : 'Black'} color bid — $${(bidCents / 100).toFixed(2)}`, cents: bidCents };
...
async function createStripeCheckout(request: Request, env: TournamentEnv): Promise<Response> {
  ...
  const body = await request.json().catch(() => ({})) as { itemId?: string; kind?: CheckoutKind; roomCode?: string; desiredColor?: BidColor; bidCents?: number };
  ...
  if (mode === 'live' && (kind === 'position_bid' || kind === 'color_bid') && env.LIVE_POSITION_BIDS !== 'enabled') return json({ error: 'Live paid color bidding is disabled. Use Stripe test mode until QQURZ explicitly enables it.' }, 403);

  const roomCode = String(body.roomCode ?? '').trim().toUpperCase();
  const bidCents = normalizeBidCents(body.bidCents);
  const desiredColor = normalizeBidColor(body.desiredColor);
  ...
  if (kind === 'color_bid' && (!desiredColor || !bidCents)) return json({ error: `Choose White or Black and enter a bid from $${(COLOR_BID_MIN_CENTS / 100).toFixed(2)} to $${(COLOR_BID_MAX_CENTS / 100).toFixed(2)}.` }, 400);

  const item = itemFor(kind, String(body.itemId ?? ''), env, bidCents, desiredColor);
  ...
  if (kind === 'position_bid' || kind === 'color_bid') success.searchParams.set('room', roomCode);
  ...
  if (kind === 'position_bid' || kind === 'color_bid') cancel.searchParams.set('room', roomCode);
...
async function verifyStripeCheckout(url: URL, env: TournamentEnv): Promise<Response> {
  ...
  const desiredColor = normalizeBidColor(payload.metadata?.desired_color);
  return json({
...
export async function handleTournamentRequest(request: Request, env: TournamentEnv): Promise<Response | null> {
  ...
  if (request.method === 'GET' && url.pathname === '/tournaments') {
    ...
    return json({
      ...
      colorBidMinCents: COLOR_BID_MIN_CENTS,
      colorBidMaxCents: COLOR_BID_MAX_CENTS,
    });
  }
