...
export default function LocalMatchChrome({ game, board, optionsOpen, onToggleOptions }: Props) {
  ...
  return <section className="local-fast-shell local-chess-layout" data-game-state={game.session.state}><div className="match-board-stack">
    ...
    {game.mode === 'ai' && game.engineStatus !== 'error' && <div className={`local-engine-state match-engine-state ${game.engineStatus}`}>{game.engineStatus === 'loading' ? 'Loading Stockfish…' : game.engineStatus === 'thinking' ? 'Stockfish thinking…' : game.engineStatus === 'ready' ? 'Stockfish ready' : 'AI preparing'}</div>}
    {game.mode === 'ai' && game.engineStatus === 'error' && <RecoveryState compact tone="warning" eyebrow="LOCAL AI" title="Stockfish is not available" body="The engine did not finish loading or could not make its move. Your local board state is still intact." primaryAction={{ label: 'Retry Stockfish', onClick: game.retryEngine }} secondaryAction={{ label: 'Play local human', onClick: () => game.setMode('human') }} />}
