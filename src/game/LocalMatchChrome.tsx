...
import { hasFreestyle } from '../premium/entitlements';
...
export default function LocalMatchChrome({ game, board, optionsOpen, onToggleOptions }: Props) {
  ...
  const viewerColor = game.humanColor ?? bottomColor;
  const viewerResult = resultTextForViewer(game.session, viewerColor) ?? game.session.result;
  ...
  return <section className="local-fast-shell local-chess-layout" data-game-state={game.session.state}><div className="match-board-stack">
    ...
    <div className="local-board-frame match-board-frame">{board}
      {game.phase === 'strategy' && <div className="local-board-overlay"><span>Strategy phase</span><strong>{formatLocalClockMs(game.session.countdownMs)}</strong><p>Green dots show legal destinations. Take your time, then start when you are ready.</p><div><button onPointerDown={() => game.setFastForward(true)} onPointerUp={() => game.setFastForward(false)} onPointerCancel={() => game.setFastForward(false)}>Hold ×4</button><button className="primary-black" onClick={game.startNow}>Start game</button></div></div>}
      {game.phase === 'ended' && viewerResult && <div className="local-board-overlay ended">{viewerResult === 'You win' && <span className="match-trophy" aria-hidden="true"><AppIcon name="trophy" /></span>}<span>Game over</span><strong className="end-title">{viewerResult}</strong><p>{hasFreestyle() ? 'Analysis and the opening explorer are included with Freestyle.' : 'Analysis and the opening explorer come with Freestyle.'}</p><button className="primary-black" onClick={game.createPosition}>New position</button></div>}
    </div>
    ...
    <div className="match-controls" aria-label="Game controls"><div className={`match-turn-note ${game.activeColor === bottomColor ? 'active' : ''}`}>{game.phase === 'strategy' ? 'Strategy phase' : game.phase === 'ended' ? viewerResult : game.activeColor === bottomColor ? 'Your move' : 'Opponent move'}</div><button onClick={game.takeBack} disabled={!game.canTakeBack}>Take back</button><button onClick={() => game.agreeDraw(bottomColor)} disabled={game.session.state !== 'ACTIVE'}>Draw</button><button className="match-resign" onClick={() => game.resign(bottomColor)} disabled={game.session.state !== 'ACTIVE'}>Resign</button><button aria-expanded={optionsOpen} onClick={onToggleOptions}>Options</button></div>
