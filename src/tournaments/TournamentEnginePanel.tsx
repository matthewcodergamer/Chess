...
export default function TournamentEnginePanel({ onOpenGame }: Props) {
  ...
  const [firstPayout, setFirstPayout] = useState(50);
  const [secondPayout, setSecondPayout] = useState(30);
  const [thirdPayout, setThirdPayout] = useState(20);
  ...
  const createDefinition = (): EngineTournamentDefinition => {
    const payoutPlaces = payoutMode === 'none' ? [] : [
      { place: 1, value: payoutMode === 'percent' ? Math.round(firstPayout * 100) : Math.round(firstPayout * 100) },
      { place: 2, value: payoutMode === 'percent' ? Math.round(secondPayout * 100) : Math.round(secondPayout * 100) },
      { place: 3, value: payoutMode === 'percent' ? Math.round(thirdPayout * 100) : Math.round(thirdPayout * 100) },
    ];
    return {
      title,
      format,
      ...
      payout: { mode: payoutMode, currency: 'USD', poolCents: Math.round(payoutPool * 100), places: payoutPlaces },
      roundCount,
  ...
  const createTournament = async () => {
    ...
    try {
      ...
      await refreshList();
      setMessage('Tournament created. Registration is live.');
  ...
  return (
    <div className="engine-tournament-shell">
      <div className="engine-tournament-head">
        <div><span className="qqurz-kicker">TOURNAMENT ENGINE</span><h2>Run real Chess960 events</h2><p>Registration, check-in, seeding, pairings, authoritative rooms, verified results and round advancement all run on the server.</p></div>
      ...
      {createOpen && (
        <section className="engine-create-panel" aria-label="Create tournament">
          <div className="engine-form-grid">
            ...
            <label><span>Entry</span><select value={entryMode} onChange={event => setEntryMode(event.target.value as 'open' | 'invite')}><option value="open">Open registration</option><option value="invite">Invite code</option></select></label>
          ...
          <div className="engine-payout-row"><label><span>Payout definition</span><select value={payoutMode} onChange={event => setPayoutMode(event.target.value as typeof payoutMode)}><option value="none">No payout</option><option value="percent">Percentage split</option><option value="fixed">Fixed amounts</option></select></label>{payoutMode !== 'none' && <><label><span>Pool (USD)</span><input type="number" min="0" value={payoutPool} onChange={event => setPayoutPool(Math.max(0, Number(event.target.value) || 0))} /></label><label><span>1st {payoutMode === 'percent' ? '%' : '$'}</span><input type="number" min="0" value={firstPayout} onChange={event => setFirstPayout(Math.max(0, Number(event.target.value) || 0))} /></label><label><span>2nd {payoutMode === 'percent' ? '%' : '$'}</span><input type="number" min="0" value={secondPayout} onChange={event => setSecondPayout(Math.max(0, Number(event.target.value) || 0))} /></label><label><span>3rd {payoutMode === 'percent' ? '%' : '$'}</span><input type="number" min="0" value={thirdPayout} onChange={event => setThirdPayout(Math.max(0, Number(event.target.value) || 0))} /></label></>}</div>
          <small className="engine-payout-note">Cash payout definitions stay test-only unless the server explicitly enables legally approved live tournament payouts and a payout provider is connected.</small>
          <button className="primary-black engine-create-submit" onClick={createTournament} disabled={busy === 'create'}>{busy === 'create' ? 'Creating…' : 'Create & open registration'}</button>
        </section>
      )}

      <div className="engine-event-layout">
        <aside className="engine-event-list" aria-label="Engine tournaments">
          {events.length ? events.map(event => <button key={event.id} className={selectedId === event.id ? 'selected' : ''} onClick={() => setSelectedId(event.id)}><span>{statusLabel(event.status)} · {formatName(event.format)}</span><b>{event.title}</b><small>{event.registered}/{event.capacity} registered · {event.currentRound ? `Round ${event.currentRound}/${event.roundCount}` : `${event.roundCount} rounds`}</small></button>) : <div className="engine-empty"><b>No engine tournaments yet.</b><span>Create the first event above.</span></div>}
        </aside>

        <section className="engine-event-detail">
          {detail ? <>
            <header><div><span className={`engine-status ${detail.status}`}>{statusLabel(detail.status)}</span><h3>{detail.title}</h3><p>{formatName(detail.format)} · {detail.timeControl.label} · {detail.capacity} seats · {detail.roundCount} rounds</p></div><div className="engine-event-actions">{canRegister && <button onClick={register} disabled={Boolean(busy)}>{busy === 'register' ? 'Registering…' : 'Register'}</button>}{detail.entryRules.mode === 'invite' && canRegister && <input aria-label="Tournament invite code" placeholder="Invite code" value={inviteCode} onChange={event => setInviteCode(event.target.value)} />}{canCheckIn && <button onClick={checkIn} disabled={Boolean(busy)}>{busy === 'checkin' ? 'Checking in…' : 'Check in'}</button>}{me?.seat && <button className="primary-black" onClick={openGame}>Open assigned game</button>}</div></header>

            <div className="engine-meta-grid"><div><span>Start</span><b>{new Date(detail.startTime).toLocaleString()}</b></div><div><span>Check-in</span><b>{detail.checkInRules.required ? `${detail.checkedIn}/${detail.registered}` : 'Not required'}</b></div><div><span>Chess960</span><b>{detail.positionPolicy.mode === 'fixed' ? `Fixed #${detail.positionPolicy.positionId}` : detail.positionPolicy.mode === 'per_round' ? 'New each round' : 'New each game'}</b></div><div><span>Organizer</span><b>{detail.organizerName}</b></div><div><span>Tie-breaks</span><b>{detail.tieBreakRules.join(' → ').replaceAll('_', ' ')}</b></div><div><span>Payout</span><b>{detail.payout.mode === 'none' ? 'None' : `${money(detail.payout.poolCents)} · ${detail.payoutStatus.replaceAll('_', ' ')}`}</b></div></div>

            {me?.participant && <div className="engine-my-status"><b>Your entry</b><span>Rating {me.participant.rating} · seed {me.participant.seed ?? 'pending'} · {me.participant.checkedInAt ? 'checked in' : detail.checkInRules.required ? 'check-in pending' : 'registered'}</span>{me.pairing && <span>{me.pairing.black ? `${me.pairing.white?.name} vs ${me.pairing.black.name}` : 'Pairing bye'} · {me.pairing.status}</span>}</div>}
