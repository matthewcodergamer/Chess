...
type NavIconName = 'menu' | 'close' | 'home' | 'trophy' | 'users' | 'cube' | 'pawn' | 'user' | 'sun' | 'board' | 'sound' | 'type' | 'accessibility';
  ...
  return <>
    ...
    <header className="qqurz-nav chess-topbar">
      ...
      <div className="qqurz-nav-end chess-nav-actions">
        ...
        <button className="nav-account-button nav-signup-button" onClick={() => run(onAccount)} {...intent('account')} aria-label="Open player account" aria-current={screen === 'account' ? 'page' : undefined}><NavIcon name="user" /><span className="nav-control-label">Account</span></button>
