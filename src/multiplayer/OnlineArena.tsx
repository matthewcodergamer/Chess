...
import MatchChat from '../ui/MatchChat';
...
export default function OnlineArena({ onClose, variant = 'friends', onShop }: Props) {
  ...
  const [colorGate, setColorGate] = useState<ColorGate>({ type: 'color_gate', enabled: false, open: true, yourOptOut: false, optedOut: 0, required: 2 });
  ...
  const pieceMotionMs = reducedMotion ? 0 : motionTokenMs('--q-motion-piece', 260);
  ...
  const handleMove = useCallback((orig: Key, dest: Key) => {
    ...
    if (parsed && pos.isLegal(parsed)) {
      playChessSound(chessSoundForSan(makeSan(pos, parsed)));
      lastMoveSoundCount.current += 1;
    }
    send({ type: 'move', uci, clientSentAt: Date.now(), clientMonotonicMs: performance.now(), clientSequence: moveSequence.current });
  ...
  useEffect(() => {
    ...
    if (gameSession.movesSan.length > lastMoveSoundCount.current) {
      const added = gameSession.movesSan.slice(lastMoveSoundCount.current);
      added.forEach(san => playChessSound(chessSoundForSan(san)));
    ...
    if (gameSession.result && gameSession.result !== lastResult.current) {
      ...
      if (seat && gameSession.winner === seat.color) playWinCelebration();
      else playChessSound('game-end');
    }
  ...
  const create = async () => {
    ...
    try { const created = await createRoom(name.trim() || 'Guest', roomTimeControl, variant === 'tournament' ? tournamentPolicy.templateId : undefined, takebacksOn); consumeHumanMatch(); setRoomCode(created.code); rememberSeat(created); setSeat(created); setRoomUrl(created.code); }
  ...
  const choosePromotion = (piece: PromotionLetter) => {
    ...
    moveSequence.current += 1;
    const uci = `${promotion.orig}${promotion.dest}${piece}`;
    if (pos) {
      const parsed = parseUci(uci);
      if (parsed && pos.isLegal(parsed)) {
        playChessSound(chessSoundForSan(makeSan(pos, parsed)));
  ...
  const buyColorBid = async (cents: 200 | 500) => {
    if (!seat || colorGate.yourOptOut) return;
  ...
  const chooseFreeToss = () => {
    if (!connectedToRoom || colorGate.yourOptOut) return;
    send({ type: 'use_coin_toss' });
    setMessage('Free quarter selected. Waiting for the other player to choose the toss too…');
  };
  ...
  const colorBidAllowed = colorPreferenceOpen;
      ...
      {snapshot ? <div className="online-game-grid match-game-grid">
        <div className="online-board-column match-board-column">
          ...
          <div className={`board-frame online-board-frame match-board-frame ${variant === 'tournament' ? 'tournament-green-board' : ''}`}>
            ...
            {gameSession && isTerminalGameState(gameSession.state) && viewerResult && <div className="board-overlay ended online-ended-overlay"><div>{viewerResult === 'You win' && <span className="match-trophy" aria-hidden="true"><AppIcon name="trophy" /></span>}<span>Game over</span><strong className="end-title">{viewerResult}</strong><small>{subscribed ? 'Analysis and the opening explorer are included with Freestyle.' : 'Analysis and the opening explorer come with Freestyle.'}</small></div></div>}
          </div>
          ...
          {colorBidAllowed && <section className="position-auction-card color-auction-card"><div><span className="eyebrow">Color preference</span><h3>Choose White or Black before the quarter toss</h3><p>Highest verified bid gets that side. If nobody buys a preference, both players choose the free quarter path and then call the normal toss.</p></div><div className="color-choice-pills" role="radiogroup" aria-label="Desired chess color"><button className={desiredColor === 'white' ? 'selected' : ''} onClick={() => setDesiredColor('white')} disabled={colorGate.yourOptOut}>White</button><button className={desiredColor === 'black' ? 'selected' : ''} onClick={() => setDesiredColor('black')} disabled={colorGate.yourOptOut}>Black</button></div><div className="auction-status"><span>Leader</span><b>{snapshot.colorAuction.leaderName ?? 'No bid yet'}</b><span>Winning side</span><b>{snapshot.colorAuction.desiredColor ? snapshot.colorAuction.desiredColor[0].toUpperCase() + snapshot.colorAuction.desiredColor.slice(1) : '—'}</b><span>Top bid</span><b>{snapshot.colorAuction.leadingBidCents ? money(snapshot.colorAuction.leadingBidCents) : '—'}</b><span>Your last bid</span><b>{snapshot.colorAuction.yourBidCents ? `${money(snapshot.colorAuction.yourBidCents)}${snapshot.colorAuction.yourBidRefunded ? ' · refunded' : ''}` : '—'}</b></div><div className="auction-actions"><button onClick={() => buyColorBid(200)} disabled={!colorBidPaymentsAvailable || Boolean(colorBidBusy) || colorGate.yourOptOut}>{colorBidBusy === 200 ? 'Opening…' : `Bid $2 for ${desiredColor === 'white' ? 'White' : 'Black'}`}</button><button onClick={() => buyColorBid(500)} disabled={!colorBidPaymentsAvailable || Boolean(colorBidBusy) || colorGate.yourOptOut}>{colorBidBusy === 500 ? 'Opening…' : `Bid $5 for ${desiredColor === 'white' ? 'White' : 'Black'}`}</button>{youLeadColorBid && <button className="settle-color-auction" onClick={() => send({ type: 'settle_color_bid' })} disabled={!connectedToRoom}>Lock winning color</button>} {!snapshot.colorAuction.leaderName && <button onClick={chooseFreeToss} disabled={!connectedToRoom || colorGate.yourOptOut}>{colorGate.yourOptOut ? 'Free toss selected' : 'Use free toss'}</button>}</div><small>{paymentMode === 'test' ? 'Stripe test mode: no real money moves.' : liveColorBidsEnabled ? 'Live color preference is enabled by server policy.' : 'Live color preference is disabled by server policy.'}</small></section>}

          {bidAllowed && <section className="position-auction-card"><div><span className="eyebrow">Position reroll</span><h3>Highest verified bid controls the next shared shuffle</h3><p>qqurzchess uses all 960 legal Chess960 starts. A new highest verified bid rerolls the same board for both players.</p></div><div className="auction-status"><span>Leader</span><b>{snapshot.auction.leaderName ?? 'No bid yet'}</b><span>Top bid</span><b>{snapshot.auction.leadingBidCents ? money(snapshot.auction.leadingBidCents) : '—'}</b><span>Your bid</span><b>{snapshot.auction.yourBidCents ? money(snapshot.auction.yourBidCents) : '—'}</b></div><div className="auction-actions"><button onClick={() => buyBid(200)} disabled={!bidPaymentsAvailable || Boolean(bidBusy)}>{bidBusy === 200 ? 'Opening…' : 'Bid $2 & reroll'}</button><button onClick={() => buyBid(500)} disabled={!bidPaymentsAvailable || Boolean(bidBusy)}>{bidBusy === 500 ? 'Opening…' : 'Bid $5 & reroll'}</button></div><small>{paymentMode === 'test' ? 'Stripe test mode: no real money moves.' : livePositionBidsEnabled ? 'Live position bidding enabled by server policy.' : 'Live position bidding is disabled.'}</small></section>}
          ...
          <div className="match-controls" aria-label="Game controls">
            ...
            {snapshot.takebacksEnabled !== false && variant !== 'tournament' && <>
              {snapshot.takebackFrom && snapshot.takebackFrom !== seat.color
                ? <>
                  <button onClick={() => send({ type: 'accept_takeback' })} disabled={!connectedToRoom || gameSession?.state !== 'ACTIVE'}>Accept take-back</button>
                  <button onClick={() => send({ type: 'decline_takeback' })} disabled={!connectedToRoom}>Decline</button>
                </>
                : <button onClick={() => send({ type: 'offer_takeback' })} disabled={!connectedToRoom || gameSession?.state !== 'ACTIVE' || !gameSession.moveNumber || snapshot.takebackFrom === seat.color}>{snapshot.takebackFrom === seat.color ? 'Take-back offered' : 'Take back'}</button>}
            </>}
          ...
          {chatOn && <MatchChat lines={snapshot.chat ?? []} selfName={playerFor(seat.color)?.name ?? name} disabled={!connectedToRoom} onSend={text => send({ type: 'chat', text })} />}
