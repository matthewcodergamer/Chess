...
async function installApi(page: Page) {
  await page.route('**/__qqurz_test_api/**', async route => {
    ...
    if (path === '/tournaments' || path === '/tournament-engine/tournaments') return json(route, { tournaments: [], serverNow: Date.now(), paymentMode: 'off', paymentConfigured: false, liveTournamentPaymentsEnabled: false, cashTournamentCheckoutMode: 'test-only', platformRakeBps: 0, premium3dPriceCents: 399, positionBidCents: [200, 500], livePositionBidsEnabled: false, colorBidCents: [200, 500], liveColorBidsEnabled: false });
